<template>
  <div>
    <PageHeader title="计量环比" />
    <el-card shadow="never" style="margin-bottom:16px">
      <el-form :inline="true">
        <el-form-item label="表计"><el-select v-model="query.meter_id" placeholder="请选择" clearable filterable style="width:180px"><el-option v-for="m in meters" :key="m.id" :label="m.name" :value="m.id" /></el-select></el-form-item>
        ...
        <el-form-item><el-button type="primary" @click="fetchData" :disabled="!query.meter_id">环比分析</el-button></el-form-item>
      </el-form>
    </el-card>
    ...
    <el-card shadow="never">
      <el-table :data="tableData" border stripe>
        ...
        <el-table-column prop="value" label="数值" align="right"><template #default="{row}"><span class="num">{{ row.value.toLocaleString('zh-CN',{minimumFractionDigits:4}) }}</span></template></el-table-column>
...
const meters = ref([]); const period1 = ref(null); const period2 = ref(null)
const value1 = ref(0); const value2 = ref(0); const name1 = ref('本期'); const name2 = ref('上期')
...
const tableData = computed(() => [
  { label: name1.value, value: value1.value, diff: diff.value, ratio: ratio.value },
  { label: name2.value, value: value2.value, diff: 0, ratio: 0 },
])

const chartOption = computed(() => ({
  tooltip: { trigger: 'axis' },
  xAxis: { type: 'category', data: [name1.value, name2.value], axisLabel: { color: '#6E6E73' } },
...
async function fetchData() {
  if (!query.meter_id || !period1.value || !period2.value) return
  try {
    const [r1, r2] = await Promise.all([
      getMeterQuery({meter_id:query.meter_id, stat_type:query.stat_type, period:'day', start_date:period1.value[0], end_date:period1.value[1]}),
      getMeterQuery({meter_id:query.meter_id, stat_type:query.stat_type, period:'day', start_date:period2.value[0], end_date:period2.value[1]})
    ])
    name1.value = '本期'; name2.value = '上期'
